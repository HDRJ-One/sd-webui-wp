...
def run_extension_installer(extension_dir):
    ...
    try:
        ...
        stdout = run(f'"{python}" "{path_installer}"', "Installing extension", "Couldn't install extension", custom_env=env, live=True)
        if stdout:
            print(stdout)
    except RuntimeError:
...
def install_extensions():
    ...
    for name in os.listdir(extensions_dir):
        ...
        run_extension_installer(ext_dir)


def requirements_met(file):
    if not os.path.isfile(file):
        return False

    try:
        result = subprocess.run([python, "-m", "pip", "check"], capture_output=True, shell=False, encoding='utf8')
        if result.returncode != 0:
            return False
...
def install_requirements():
    ...
    requirements_file_for_npu = "requirements_npu.txt"

    if not requirements_met(requirements_file):
        run_pip(f"install -r {requirements_file}", "requirements")

    if not requirements_met(requirements_file_for_npu):
        run_pip(f"install -r {requirements_file_for_npu}", "requirements for NPU")
...
def prepare_environment():
    ...
    if args.skip_extensions_install:
    ...
    if not args.skip_install:
        run_pip(f"install -r requirements.txt", "installing requirements", live=True)

    startup_timer.record("complete")
